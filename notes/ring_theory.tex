...
\begin{enumerate}
  ...
  \item
    Es gilt $S = \{ \sum_{i=1}^n \varepsilon_i x_{i,1} \dotsm x_{i,m_i} \suchthat n, m_i \in \Natural, \varepsilon_i = \pm 1, x_{i,j} \in X \}$.
\end{enumerate}
...
\begin{theorem}[Universelle Eigenschaft des Polynomrings]
  ...
  \[
    ...
    = \sum_{\alpha \in \Natural^n} a_\alpha s_1^{\alpha_1} \dotsm s_n^{\alpha_n} \,.
  \]
  In anderen Worten:
  Es ergibt sich eine Bijektion
  \begin{align*}
...
\begin{definition}
  Ein \emph{euklidischer Ring} ist ein Integritätsbereich $R$ zusammen mit einer \emph{Gradabbildung} $\delta \colon R \smallsetminus \{0\} \to \Natural$, so dass es für alle $f, g \in R$ mit $g \neq 0$ Elemente $q, r \in R$ gibt, so dass
  \[
    f = qg + r \,,
    \qquad
...
\label{definition: ufd}
  ...
  \begin{itemize}
    ...
    \item
      ...
      Falls $r = \varepsilon' p'_1 \dotsm p'_m$ eine weitere solche Zerlegung ist, so gilt $n = m$, und es gibt Einheiten $\delta_1, \dotsc, \delta_n \in \unitgroup{R}$ uond eine Permutation $\pi \in S_n$ mit $p'_i = \delta_i p_{\pi(i)}$ für alle $i = 1, \dotsc, n$.
  \end{itemize}
\end{definition}
