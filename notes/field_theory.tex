...
Ist $\varphi \colon K \to L$ ein Körperhomomorphismus, so induziert $\varphi$ also einen Körperisomorphismus $\induced{\varphi} \colon  K \to \im(\varphi)$.
...
\begin{example}
  Für jeden $\Complex$-Vektorraum $V$ gilt $\dim_\Real(V) = 2 \dim_\Complex(V)$.
\end{example}
...
\begin{definition}
  ...
  \[
    ...
    =         L_2(L_1) \,,
  \]
  alse der kleinste Zwischenkörper der Erweiterung $L/K$, der $L_1$ und $L_2$ enthält.
\end{definition}
...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item
      Sind $L_1/K$, $L_2/K$ algebraisch mit $K$-Basen $(a_i)_{i \in I}$ und $(b_j)_{j \in J}$, so ist $(a_i b_j)_{i \in I, j \in J}$ ein $K$-Erzeugendensystem von $L_1 L_2$.
    \item
...
\begin{definition}
  Ein $K$-linearer Körperhomomorphismus $\varphi \colon L_1 \to L_2$ ist ein \emph{$K$-Homomorphismus}.
...
\begin{example}
  Fasst man zwei Körper $L_1, L_2$ gleicher Charakteristik als Körpererweiterungen $L_1/P$, $L_2/P$ des gemeinsamen Primkörpers $P$ auf, so ist jeder Körperhomomorphismus $\varphi \colon L_1 \to L_2$ bereits $P$-linear, und somit ein $P$-Homomorphismus:
  Es gibt nämlich nur genau einen Körperhomomorphismus $P \to L_2$, weshalb das Diagramm
  \[
    \begin{tikzcd}
        L_1
        \arrow{rr}[above]{\varphi}
...
\begin{definition}
  Es ist $\Aut{L/K} \defined \{\text{$K$-Automorphismen $L \to L$}\}$ die \emph{Automorphismengruppe} der Erweiterung $L/K$j
\end{definition}
...
\begin{lemma}[Forsetzungssätze für algebraische Abschlüsse]
  ...
  \begin{enumerate}
    \item
      Ist $L/K$ eine algebraische Körpererweiterung und $L'$ ein algebraisch abgeschlossener Körper, so setzt sich jeder Körperhomomorphismus $\varphi \colon K \to L'$, also Einbettung $K \hookrightarrow L'$ zu einem Körperhomomorphismus $\psi \colon L \to L'$, also einer Einbettung $L \hookrightarrow{L'}$, fort d.h.\ es gilt $\restrict{\psi}{K} = \varphi$.
      \[
        \begin{tikzcd}
            L
...
\begin{lemma}
  ...
  Es sei $\varphi \colon K \to K'$ ein Körperisomorphismus und $\varphi_* \colon K[t] \to K'[t]$, $\sum_{i=0}^n a_i t^i \mapsto \sum_{i=0}^n \varphi(a_i) t^i$ der induzierte Ringhomomorphismus.
...
Da $L$ von den Nullstellen der $f_i$ erzeugt wird, ist somit das gesamte Bild $\varphi(L)$ unabhängig von der Wahl von $f_i$.

\begin{example}
  Für jeden $\Real$-Homomorphismus $\varphi \colon \Complex \to \Complex$ git $\varphi(\{i, -i\}) = \{i, -i\}$.
\end{example}
...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item
      Jedes irreduzible Polynom $f \in K[t]$, dass in $L$ eine Nullstelle hat, zerfällt in $L$ bereits in Linearfaktoren.
  \end{enumerate}
\end{proposition}
